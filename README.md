...
- **`forum`(推荐,需论坛群)** —— 把 bot 拉进一个开启「话题」的群、设为管理员并给「管理话题」权限。每轮在群里发**一条索引消息**,每条新帖一个按钮;**你点了哪条,才为它单独建一个话题(Topic)并起草回复**——没点的不建、不花钱。点按钮即跳进该帖的**专属窗口**,在里面直接打字(如 `更毒一点`/`更短`)就能让它**基于你的话**重写;话题里有 **🗑 删除话题** 按钮,**前一天的话题每天自动清理**(`poll.topic_ttl_hours`,默认 24h)。
